...
const BlogCard = ({ item }: { item: any }) => {
  return <div className="bg-white rounded-xl shadow-md overflow-hidden blog-card">
    ...
    <div className="p-6">
      ...
      <Link href={`/blog/${item.id}`} className="text-xl font-bold mb-3">{item.title}</Link>
      <p className="text-gray-600 mb-4">{item.short_description}</p>
      <Link href={`/blog/${item.id}`} className="flex items-center gap-2 text-blue-500 font-medium flex items-center">
        Read more
