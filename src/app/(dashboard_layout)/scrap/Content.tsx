    ...
    for await (const itm of scrap) {
      ...
      if (itm.category) {
        const afunction = async () => {
          await get('categories', [{ field: 'scrapedId', operator: '==', value: itm.category }]).then(res => {
            let fres: any
            if (res.docs) {
              fres = res.docs.map((itm: any) => {
                return {
                  ...itm.data(),
                  id: itm.id
                }
      ...
      itm.scrapedId = String(itm.id)
      delete itm.id
      res.push({ ...itm, scrapedAt: new Date().toISOString() })
    }
        ...
        <ul className="flex border-b mb-3">
          <li>
            <button
              ...
              className={`px-4 py-2 -mb-px border-b-2 transition-colors duration-200 ${tab === "export"
          ...
          <li>
            <button
              ...
              className={`px-4 py-2 -mb-px border-b-2 transition-colors duration-200 ${tab === "import"
                  ? "border-blue-500 text-blue-500 font-semibold"
                  : "border-transparent text-gray-500 hover:text-blue-500"
                }`}
            >
