...
export default function Content() {
    ...
    const remove=(id:any)=>{
     ...
     function convertUrlsToAnchors(inputString:string) {
        const urlPattern = /(https?:\/\/[^\s]+)/g; // Regex pattern to match URLs
        return inputString.replace(urlPattern, (url) => `<a href="${url}" target="_new">${url}</a>`);
    }
    ...
    const columns: MkTableColumn[] = [
        ...
        },
        {
            ...
            render: (row) => {
                return <span className='max-w-[450px] inline-block break-all whitespace-pre-wrap ancr-color' dangerouslySetInnerHTML={{__html:convertUrlsToAnchors(row?.message)}}></span>
            }
        },
        {
            key: 'user', name: 'User',
