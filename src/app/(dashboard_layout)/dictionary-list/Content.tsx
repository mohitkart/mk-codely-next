    ...
    const columns: MkTableColumn[] = [
         {
            ...
            render: (row) => {
                return <span className="capitalize">{row?.english}</span>
            }
        },
        {
            key: 'hindi', name: 'Hindi', sort: true,
            render: (row) => {
                return <span className="capitalize">{row?.hindi}</span>
            }
        },
        {
        ...
        return list
            ?.filter((item: any) => {
                ...
                if (filters.search) {
                    ...
                    const hindi = item.hindi?.toLowerCase() || '';
                    const english = item.english?.toLowerCase() || '';
                    if (!hindi.includes(searchValue) && !english.includes(searchValue))
                        return false;
                }
