...
import { PAGE_TABLE, PAGE_URL } from "./shared";
...
export default function Content() {
    ...
    const columns: MkTableColumn[] = [
        ...
        {
            key: 'comment', name: 'Comment', sort: true,
            render: (row) => {
                return <>{row?.comment}</>
            }
        },
            ...
            render: (itm) => {
                return <>
                    <div className="flex space-x-2">
                        <Link href={`${envirnment.frontUrl}blog/${itm.blogid}`} target="_blank" className="action-btn" title="View">
