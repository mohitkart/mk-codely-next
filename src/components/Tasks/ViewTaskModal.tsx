...
import { truncate, TruncateHtml } from '~/utils/shared';
...
const ViewTaskModal = forwardRef<ViewTaskModalRef, ViewTaskModalProps>(({
  ...
  const loadSubTasks = async () => {
    if (user) {
      let conditions: FirestoreConditions[] = [
        { field: 'parent', operator: '==', value: task.id }
      ]
      const res = await getTasks(table, conditions);
      if (res.success) {
        let data = res.data.map((itm: Task) => ({ ...itm, date: fireDateParse(itm.date) })).sort((a: any, b: any) => {
          return new Date(b.createdAt)?.getTime() - new Date(a.createdAt)?.getTime()
        })
        setSubTask(data)
      }
    } else {
      let arr=tasks.filter(itm=>itm.parent==task.id)
      setSubTask([...arr])
    }
  }
        ...
        content={<>
          <div className="py-4 max-h-[calc(100vh-150px)] overflow-auto">
            ...
            <div className="flex items-center text-sm text-gray-500 mb-4">
              <span className="material-icons mr-1 text-gray-400">schedule</span>
            ...
            <div className="mb-4">
              <div className="flex justify-between items-center mb-2">
                ...
                {task?.addedBy == user?.id ? <>
                  ...
                  >
                    <span className="material-icons mr-1 text-sm">add</span> Add Subtask
                  </button>
                </> : <></>}
              </div>
