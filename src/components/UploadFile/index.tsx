...
const UploadFile = ({ multiple = false, modal = 'default', type = 'image', label = 'Upload', accept = 'image/*', value = '', result = () => { } }: Props) => {
    ...
    const imgPath = (p: string) => {
        return imageModel.getImage(p, modal)
    }
        ...
        <div>
            ...
            {multiple ? <>
                <div className="imagesRow">
                    {image && image.map((itm: any, i: any) => {
                        return <div className="imagethumbWrapper" key={i}>
                            {type == 'audio' ? <>
                                <AudioHtml src={imgPath(itm)} />
                            </> : <></>}
                            {type == 'video' ? <>
                                <VideoHtml src={imgPath(itm)} />
                            </> : <></>}
                            {type == 'image' ? <>
                                <img src={imgPath(itm)} className="thumbnail" />
                            </> : <></>}
                ...
                {image ? <div className="imagethumbWrapper">
                    {type == 'audio' ? <>
                        <AudioHtml src={imgPath(image)} />
                    </> : <></>}
                    {type == 'video' ? <>
                        <VideoHtml src={imgPath(image)} />
                    </> : <></>}
                    {type == 'image' ? <>
                        <img src={imgPath(image)} className="thumbnail" />
                    </> : <></>}
                    ...
                    <a href={imgPath(image)} className="d-block" target="_blank">View and download</a>
                </div> : <></>}
            </>}
        </div>
